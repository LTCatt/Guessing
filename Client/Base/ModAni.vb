                               ...
                               Do While True
                                   ...
                                   If ModeDebug Then
                                       If MathRange(AniLastTick - AniFPSTimer, 0, 10000) >= 500 Then
                                           ...
                                           AniFPSCounter = 0
                                           AniFPSTimer = AniLastTick
                                           'If AniFPS <= 32 Then Log("[UI] FPS 过低：" & AniFPS, LogLevel.Developer)
                                           'If AniCount >= 16 Then Log("[UI] 过多的动画数：" & AniCount, LogLevel.Developer)
                                       End If
                                       AniFPSCounter += 2
                                   End If
                                   '执行动画
                                   RunInUiWait(Sub()
                                                   ...
                                                   '#End If
                                                   If RandomInteger(0, 64 * If(ModeDebug, 5, 30)) = 0 AndAlso ((AniFPS < 62 AndAlso AniFPS > 0) OrElse AniCount > 4) Then
                                                       Log("[Report] FPS " & AniFPS & ", 动画 " & AniCount)
                                                   End If
                                               End Sub)
Sleeper:
                                   '控制 FPS
