...
#Region "文件"
    ...
    Public Function GetPathFromFullPath(FilePath As String) As String
        ...
        If FilePath.EndsWith("\") OrElse FilePath.EndsWith("/") Then
            ...
            GetPathFromFullPath = Left(FilePath, FilePath.LastIndexOfAny({"\", "/"}) + 1) & If(IsRight, "\", "/")
...
Re:
        Try
            ...
            Dim MD5 As Security.Cryptography.MD5 = New Security.Cryptography.MD5CryptoServiceProvider()
...
Re:
        Try
            ...
            '获取 SHA1
            Dim file As New FileStream(FilePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)
            Dim sha1 As Security.Cryptography.SHA1 = New Security.Cryptography.SHA1CryptoServiceProvider()
...
#Region "文本"
    ...
    Public Function GetString(Ex As Exception, Optional IsLine As Boolean = True, Optional ShowAllTrace As Boolean = False) As String
        ...
        Else
            ...
            Dim TypeName As String = Ex.GetType.FullName
            Dim Desc As New List(Of String)
    ...
    ''字符串压缩
    'Public Function GZipCompress(Source As Byte()) As Byte()
    ...
    '    Return MS.ToArray()
    'End Function
    'Public Function GZipDecompress(Source As Byte()) As Byte()
    '    Dim MS As New MemoryStream(Source)
    '    Dim compressedzipStream As New GZipStream(MS, CompressionMode.Decompress)
    '    Dim outBuffer As New MemoryStream()
    '    Dim block(1024) As Byte
    '    While True
    '        Dim bytesRead = compressedzipStream.Read(block, 0, block.Length)
    '        If bytesRead <= 0 Then
    '            Exit While
    '        Else
    ...
    Private Function SecureKey(Key As String) As String
        If String.IsNullOrEmpty(Key) Then
            Return "@;$ Abv2"
        Else
            Return Mid(StrFill(GetHash(Key), "X", 8), 1, 8)
        End If
    End Function
    ''' <summary>
    ''' 加密字符串。
    ...
    Public Function SecureAdd(SourceString As String, Optional Key As String = "") As String
        ...
        Dim btIV As Byte() = Encoding.UTF8.GetBytes("95168702")
        Dim des As New Security.Cryptography.DESCryptoServiceProvider
        Using MS As New MemoryStream
            Dim inData As Byte() = Encoding.UTF8.GetBytes(SourceString)
            Using cs As New Security.Cryptography.CryptoStream(MS, des.CreateEncryptor(btKey, btIV), Security.Cryptography.CryptoStreamMode.Write)
    ...
    Public Function SecureRemove(SourceString As String, Optional Key As String = "") As String
        ...
        Using MS As New MemoryStream
            ...
            Using cs As New Security.Cryptography.CryptoStream(MS, des.CreateDecryptor(btKey, btIV), Security.Cryptography.CryptoStreamMode.Write)
                cs.Write(inData, 0, inData.Length)
                cs.FlushFinalBlock()
    ...
    'RSA 加解密
#If DEBUG Then
    Private Const RsaPrivateKey As String = "<RSAKeyValue><Modulus>0L/cZoJUyBRAIE8OKiG8+qYOytXD+azFCBsmOuQraQGAQ1S/R1Frckd3/Sn+Zsx9aD6U2f84SdWMDlrRY9DfhQ==</Modulus><Exponent>AQAB</Exponent><P>2JWqHX3/Y6LixOjbdUDGMzmsONtmVLOwalqMWr3Vz9s=</P><Q>9r0rERCC5Vc5Ll9Cyo85N81aN/Cu2noqUZhYO0rOXB8=</Q><DP>IqkQ6FdZn6BZmhmJxWHwVhJMtWynG7cF7H9BJuRqazE=</DP><DQ>DiIj12RkuxJV+iwGvWea1XFz24Rz35vj1KyvUSYlz+0=</DQ><InverseQ>BDkeLZAzo2La5q2zw4vmvE9+5MAZMr4rFBHa9y4RDSc=</InverseQ><D>MkdT5ZjUpNBD1cRUzA8OPmzKuU4dgsB7QIWRgrO8RPD5BCQljm3NhpYsw5KNmDLSj3n/LR7GjOevQj4K893jbQ==</D></RSAKeyValue>"
    ''' <summary>
    ''' 使用 RSA 私钥签名字符串。
    ''' </summary>
    Public Function SecureAddRsa(SourceString As String) As String
        Dim Rsa As New RSACryptoServiceProvider(512)
        Rsa.FromXmlString(RsaPrivateKey)
        Return Convert.ToBase64String(Rsa.SignData(Encoding.Default.GetBytes(SourceString), GetType(SHA256)))
    End Function
#End If
    '上一次修改在 Alpha 1.3.8 (167)
    Public RsaPublicKey As String = "<RS!AKeyValu!e><Mo!dul!us>0L/cZoJUyBRAIE8OKiG8$qYOytXD$azFCBsmOuQra"
    ''' <summary>
    ''' 使用 RSA 公钥验证字符串。
    ''' </summary>
    Public Function SecureRemoveRsa(SourceString As String, Sign As String) As Boolean
        Try
            Dim Rsa As New RSACryptoServiceProvider(512)
            Rsa.FromXmlString(RsaPublicKey.Replace("!", "").Replace("$", "+") & "/R1Frckd3/Sn+Zsx9aD6U2f" & Math.Round(84) & "SdWMDlrRY9DfhQ==</Modulus><Exponent>AQAB<\Exponent><\RSAKeyValue>".Replace("\", "/"))
            Return Rsa.VerifyData(Encoding.Default.GetBytes(SourceString), GetType(SHA256), Convert.FromBase64String(Sign))
        Catch
            Return False
        End Try
    End Function
...
#Region "系统"
    ...
    Public Delegate Sub EventDelegate(sender As Object, e As EventArgs)
...
NextElement:
    ...
    End Function
    ''' <summary>
    ''' 获取腾讯云 CDN TypeA 的实际下载地址。
    ''' 只处理以 {CDN} 标记结尾的链接，否则直接返回原链接。
    ''' </summary>
    Public Function GetCdnTypeA(UrlWithMark As String)
        '只处理以 {CDN} 结尾的链接
        If Not UrlWithMark.EndsWith("{CDN}") Then Return UrlWithMark
        Dim Url As String = UrlWithMark.Replace("{CDN}", "").Replace(" ", "%20")
        '获取实际下载地址
        Dim RandomText As String = StrFill(RandomInteger(0, Integer.MaxValue - 2).ToString("x"), "0", 8)
        Dim Password As String = SecureRemove("VwHB1je1uabAr0gKijpFaQ==", "CDN")
        Dim Timestamp As String = GetUnixTimestamp()
        Dim FilePath As String = Url.Substring(Url.IndexOf("://") + 3)
        FilePath = FilePath.Substring(FilePath.IndexOf("/"))
        Dim Hash As String = GetStringMD5(Join({FilePath, Timestamp, RandomText, "0", Password}, "-"))
        Return Url & "?sign=" & Join({Timestamp, RandomText, "0", Hash}, "-")
    End Function
    ...
    Public Sub RunInUi(ThreadStart As [Delegate], ParamArray Param As Object())
        If RunInUi() Then
            ...
            Catch ex As Exception
                Log(ex, "在 UI 线程执行代码出错", LogLevel.Feedback)
            End Try
        Else
    ...
    Public Function Sort(List As ArrayList, SortRule As CompareThreadStart) As ArrayList
    ...
    ''' </summary>
    ''' <param name="SortRule">传入两个对象，若第一个对象应该排在前面，则返回 True。</param>
    Public Function Sort(Of T)(List As List(Of T), SortRule As CompareThreadStart) As List(Of T)
